...
const NOTE_NAMES: Record<number, string> = { 0: 'C', 2: 'D', 4: 'E', 5: 'F', 7: 'G', 9: 'A', 11: 'B' };
const NOTE_COLORS: Record<number, string> = {
  0: '#FF5C24', 2: '#2496FF', 4: '#00FF9E', 5: '#FF245B', 7: '#8557FF', 9: '#FFD33F', 11: '#2496FF',
};
const SHARP_H = 56;
...
export const PianoKeyboard = memo(function PianoKeyboard({
  numberOfOctaves = 2, onNoteOn, onNoteOff,
  externalPressedNotes = new Set(), highlightColor = '#00FF9E', showNoteNames = false,
}: PianoKeyboardProps) {
  ...
  const overlayToKey = useCallback((visualIdx: number): number | null => {
  ...
  const handleNativePress = useCallback((visualIdx: number) => {
  ...
  const handleNativeRelease = useCallback((visualIdx: number) => {
    const key = overlayToKey(visualIdx);
    if (key == null) return;
  ...
  const handleIn = useCallback((k: number) => {
    setPressed(prev => new Set(prev).add(k));
    onNoteOn?.(k);
  }, [onNoteOn]);

  const handleOut = useCallback((k: number) => {
  ...
  const renderOctave = (octave: number) => {
    ...
    return (
      <View key={octave} style={styles.octave}>
        <View style={styles.sharpRow} pointerEvents={useNative ? 'none' : 'auto'}>
          {sk.map(k => {
            ...
            const pressProps = useNative ? {} : { onPressIn: () => handleIn(k), onPressOut: () => handleOut(k) };
            return (
              <KeyWrapper
                key={k}
                {...pressProps}
                style={[styles.sharpKey, sq ? styles.sharpSq : styles.sharpRect,
          ...
          {nk.map(k => {
            ...
            return (
              <KeyWrapper
                key={k}
                {...pressProps}
                style={[styles.natKey, { backgroundColor: active ? highlightColor : 'rgba(247,247,247,0.8)' }]}
              >
                {showNoteNames && NOTE_NAMES[s] && NOTE_COLORS[s] && (
                  <View style={[styles.badge, { backgroundColor: NOTE_COLORS[s] }]}>
                    <Text variant="extraSmall" color="#FFF" bold style={styles.badgeTxt}>{NOTE_NAMES[s]}</Text>
                  </View>
                )}
              </KeyWrapper>
            );
  ...
  return (
    <View style={[isPhone ? styles.vCont : styles.hCont, styles.relative]} accessibilityLabel="Piano keyboard">
...
const styles = StyleSheet.create({
  ...
  vCont: { minHeight: 330 },
  hCont: { flexDirection: 'row', minHeight: 330 },
  octave: { flex: 1 },
  ...
  natKey: { flex: 1, borderWidth: 1, borderColor: '#000', justifyContent: 'flex-end', alignItems: 'center', paddingBottom: 4 },
