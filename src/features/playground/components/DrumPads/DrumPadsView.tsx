...
export const DrumPadsView = memo(function DrumPadsView({
  samples, onPadPress, onPadRelease, externalPressedNotes = new Set(), highlightColor = '#00FF9E',
}: DrumPadsViewProps) {
  ...
  const handleNativeRelease = useCallback((visualIdx: number) => {
    const sampleIdx = visualToSample(visualIdx);
    setPressedPads(prev => { const n = new Set(prev); n.delete(sampleIdx); return n; });
  ...
  const handlePressOut = useCallback((idx: number) => {
    setPressedPads(prev => { const n = new Set(prev); n.delete(idx); return n; });
    onPadRelease?.(idx);
  }, [onPadRelease]);
  ...
  return (
    <View style={styles.container} accessibilityLabel="Drum pads">
      {/* Visual grid */}
      <View style={styles.grid} pointerEvents={Platform.OS === 'ios' ? 'none' : 'auto'}>
        {[0, 1, 2, 3].map(row => (
          <View key={row} style={styles.row}>
            {[0, 1, 2, 3].map(col => {
              const idx = sampleIndexFromGrid(row, col);
              const sample = samples[idx];
              const isActive = pressedPads.has(idx) || externalPressedNotes.has(idx);

              if (!sample) {
                return <View key={col} style={[styles.pad, { backgroundColor: colors.mcBlack2 }]} />;
              }
              ...
              if (Platform.OS !== 'ios') {
                ...
                return (
                  <Pressable
                    ...
                    style={[styles.pad, { backgroundColor: isActive ? highlightColor : colors.mcBlack3 }]}
                  >
                    <Text variant="extraSmall" color="rgba(255,255,255,0.5)" numberOfLines={2} style={styles.label}>
                      {sample.fileName}
                    </Text>
                  </Pressable>
                );
              ...
              return (
                <View key={col} style={[styles.pad, { backgroundColor: isActive ? highlightColor : colors.mcBlack3 }]}>
                  <Text variant="extraSmall" color="rgba(255,255,255,0.5)" numberOfLines={2} style={styles.label}>
                    {sample.fileName}
                  </Text>
                </View>
...
const styles = StyleSheet.create({
  container: { aspectRatio: 1, position: 'relative' },
  grid: { flex: 1, gap: 1 },
