  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.chapterType}>{chapter.type.toUpperCase()} • Step {stepIndex + 1}/{steps.length}</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        {step.content.map((block, i) => (
          <ContentBlockRenderer key={`${step.id}-${i}`} block={block as any} basePath={basePath} />
        ))}
      </ScrollView>
      ...
      <View style={styles.nav}>
        ...
        <Pressable
          ...
          accessibilityRole="button"
        >
          <Text style={styles.primaryBtnText}>{isLast ? 'Complete' : 'Next'}</Text>
        </Pressable>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  dots: { flexDirection: 'row', gap: 6, paddingHorizontal: makeSpacing(4), marginBottom: makeSpacing(2) },
  ...
  nav: { flexDirection: 'row', padding: makeSpacing(4), paddingBottom: makeSpacing(8) },
  spacer: { flex: 1 },
  primaryBtn: { backgroundColor: '#FF5C24', paddingVertical: 14, paddingHorizontal: 28, borderRadius: 6 },
  primaryBtnText: { color: '#F7F7F7', fontSize: 16, fontWeight: '600' },
  secondaryBtn: { borderWidth: 1, borderColor: 'rgba(247,247,247,0.3)', paddingVertical: 14, paddingHorizontal: 28, borderRadius: 6 },
