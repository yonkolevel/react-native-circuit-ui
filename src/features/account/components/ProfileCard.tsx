      ...
      <View style={styles.info}>
        ...
        {onEditProfile && (
          <Pressable onPress={onEditProfile} style={styles.link} accessibilityRole="button" accessibilityLabel="Edit profile">
            <Text variant="small" color={colors.mcBlue}>Edit Account</Text>
          </Pressable>
        )}

        {onLogout && (
          <Pressable onPress={onLogout} style={styles.link} accessibilityRole="button" accessibilityLabel="Log out">
            <Text variant="small" color={colors.mcPink}>Log out</Text>
          </Pressable>
        )}
      </View>
    </View>
...
const styles = StyleSheet.create({
  container: {
    flexDirection: 'row', alignItems: 'center', gap: makeSpacing(3),
    padding: makeSpacing(4), borderRadius: 5,
